...
The flow runs every 30 minutes to respect Discourse API rate limits. You can adjust the schedule in the flow settings if needed.

## Security Note

The API key is stored in environment variables and only used server-side in the Directus flow. It's never exposed to the client.
