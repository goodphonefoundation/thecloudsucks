...
## Note-Taking Apps and E2EE (Sync and Data Protection)

**Traditional note services** (Evernote, OneNote, Google Keep): notes stored in plaintext on provider's servers. Provider can read, index (for search features), analyze (for ads), and must comply with legal requests. Convenient sync but zero privacy.

**E2EE note-taking**: notes encrypted on device before sync. Server stores ciphertext. Only your devices (with encryption key) can decrypt. Provider cannot read notes.

**Sync challenges with E2EE**:
- **Conflict resolution**: two devices edit offline, then sync. Encrypted data harder to merge. Apps use CRDTs (Conflict-free Replicated Data Types) or last-write-wins.
- **Search**: server-side search impossible (encrypted). Must sync all notes locally for full-text search. Encrypted metadata (titles, tags) may be indexed if not obfuscated.
- **Collaboration**: sharing note requires encrypting with recipient's key. Key distribution and trust (verify recipient's public key) become issues.

**Key derivation**: passphrase → KDF (Argon2, PBKDF2) → master key. Master key encrypts per-note keys or entire note database. Stronger KDF (high iterations) slows brute-force but adds latency at unlock.

**Local vs cloud**: 
- **Local-only** (Org-mode, plain Markdown + Syncthing): notes never leave your control. Sync via self-hosted or P2P. No third-party risk but requires setup.
- **Cloud E2EE** (Standard Notes, Notesnook): convenience of cloud sync with encryption. Trust provider's implementation and that they don't backdoor clients.

**Example: Standard Notes E2EE**:
1. User creates account, derives encryption key from password (PBKDF2, 100k+ iterations).
2. Create note: encrypt content with AES-256.
3. Sync encrypted note to Standard Notes servers.
4. Another device: fetch encrypted note, decrypt locally with derived key.
5. Server sees ciphertext, timestamps, user ID. Cannot read note content.

**Markdown and plaintext advantages**: E2EE note apps often use Markdown (Standard Notes, Joplin, Notesnook). Plaintext format ensures data portability: export notes without vendor lock-in. Encrypted container around standard format.
