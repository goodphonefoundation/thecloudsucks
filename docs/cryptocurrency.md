---
meta_title: "Private Cryptocurrency Blockchains - The Cloud Sucks"
...
## Cryptocurrency Privacy (Blockchain Transparency, Ring Signatures, Stealth Addresses)

**Bitcoin's transparency problem**: all transactions public on blockchain. Every address, amount, timestamp visible forever. Blockchain analysis firms (Chainalysis, Elliptic) trace coins from origin to destination. Link addresses to real identities via: exchange KYC, merchant purchases, IP addresses (if you run a node), clustering heuristics (inputs = same owner).

**Privacy coins** (Monero, Zcash): cryptographic techniques hide transaction details. Three pillars: sender anonymity, receiver anonymity, amount obfuscation.

**Monero's privacy mechanisms**:
1. **Ring signatures**: your transaction mixed with 15 others (ring size). Observers can't tell which one is real. All 16 appear equally valid. Computational impossibility to determine true signer.
2. **Stealth addresses**: one-time destination address per transaction. Sender uses recipient's public key to derive unique address. Only recipient can detect payment (scanning blockchain with view key). Outsiders can't link transactions to recipient's published address.
3. **RingCT (Ring Confidential Transactions)**: hides transaction amounts. Uses Pedersen commitments (cryptographic proof: inputs = outputs, without revealing values). Amount encrypted; network validates without seeing plaintext.
4. **Kovri (deprecated; replaced by Dandelion++)**: obfuscates transaction origin IP. Broadcast transactions in phases to prevent timing correlation.

**Zcash and zk-SNARKs**: zero-knowledge proofs. Prove transaction validity without revealing sender, recipient, or amount. Two address types: transparent (t-addresses, like Bitcoin) and shielded (z-addresses, private). Privacy only works with z-to-z transactions; most Zcash usage is transparent (only ~5% fully shielded).

**Monero tracing challenges**: ring signatures have statistical weaknesses. If adversary controls many network nodes, can correlate timing/IP. CipherTrace claims partial tracing (probabilistic, not definitive). Heuristics: wallet fingerprinting, ring member reuse, temporal analysis. Improves with larger ring size (now 16, previously smaller).

**Transaction graph privacy**: even if individual transaction hidden, patterns emerge. Exchange KYC links address to identity. If you spend non-KYC Monero to merchant who later exchanges for fiat, merchant's KYC creates trail. Best practice: multiple hops between acquisition and spend; use different wallets.

**Nodes and privacy**: running your own node = no third party learns your transaction times, addresses, or IP. Light wallets leak: send view key to node operator (sees all your transactions) or send bloom filter/addresses (probabilistic privacy leak). Self-custody + self-hosted node = best Monero privacy.

**Example: Bitcoin tracing**:
1. Buy 1 BTC on Kraken (KYC: link address A to your name).
2. Transfer to address B (owned by you but not KYC'd).
3. Buy something from merchant; send from B to merchant address C.
4. Blockchain analysis: A → B → C all visible. Kraken knows A = you. Heuristic: B likely yours (change address, timing). Merchant C identified via website leak/data breach.
5. Result: Kraken → you → merchant trail reconstructed.

With Monero: each step uses ring signatures + stealth addresses. Observer cannot definitively link A → B → C.
