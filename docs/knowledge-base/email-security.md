---
meta_title: "Why Email Isn't the Best Choice for Privacy and Security - The Cloud Sucks"
...
The [Web Key Directory (WKD)](https://wiki.gnupg.org/WKD) standard allows email clients to discover the OpenPGP key for other mailboxes, even those hosted on a different provider. Email clients which support WKD will ask the recipient's server for a key based on the email address' domain name. For example, if you emailed `dominic@thecloudsucks.guide`, your email client would ask `thecloudsucks.guide` for Dominic's OpenPGP key, and if `thecloudsucks.guide` has a key for that account, your message would be automatically encrypted.
