...
## Understanding Threat Categories (Technical Context)

**Attack surfaces**: every interface between your system and the world (network, filesystem, applications, hardware). More surface = more attack vectors. Reducing unnecessary exposure (disable services, remove unused apps, firewall rules) shrinks attack surface.

**Passive vs active adversaries**: 
- **Passive**: observes traffic, collects metadata, exploits existing vulnerabilities. Example: ISP logging, mass surveillance, opportunistic malware.
- **Active**: targets you specifically. Custom exploits, social engineering, physical access. Example: state-sponsored APT, stalkers, corporate espionage.

**Defense in depth**: layered security. If one layer fails, others remain. Example: full-disk encryption + app sandboxing + network firewall + password manager. Attacker must breach multiple defenses.

**Compartmentalization**: isolate activities/data. Breach in one compartment doesn't compromise others. Technical implementations: separate user accounts, VMs (Qubes OS), containers, browser profiles, distinct email addresses per context.

**Metadata vs content**: content is what you say; metadata is who, when, where, how often. E2EE protects content. Metadata often unprotected and highly revealing ("metadata is the new content"). Traffic analysis, social graph inference, timing correlation all use metadata.

**Trust boundaries**: points where you must trust a party (OS vendor, VPN provider, email host). Minimize trust: use E2EE (don't trust server), open-source (verify code), self-host (trust yourself), hardware security (TPM, secure enclave isolate secrets).
