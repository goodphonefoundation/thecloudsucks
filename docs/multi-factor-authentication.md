...
## MFA Mechanisms (TOTP, HOTP, FIDO2/WebAuthn)

**TOTP (RFC 6238)**: `code = HMAC-SHA1(secret, floor(current_time / 30)) mod 10^6`. Both sides compute the same code using a shared secret and synchronized clocks (±1 time window tolerance).

**HOTP (RFC 4226)**: counter-based OTP. `code = HMAC-SHA1(secret, counter) mod 10^6`. Counter increments with each use. Less common than TOTP.

**SMS/Email OTP**: codes sent via SMS or email. Vulnerable to SIM swaps, email compromise, interception. Avoid if better options exist.

**FIDO2/WebAuthn (hardware keys, platform authenticators)**: public-key based. Private key never leaves device. Phishing-resistant because authentication is origin-bound (domain-specific). Recommended over TOTP when available (see [security keys](security-keys.md)).

**Passkeys**: a consumer-friendly term for FIDO2 credentials that can be synced (e.g., via iCloud Keychain, password managers). Combines convenience with strong security.

**Why TOTP is phishing-prone**: attacker proxies the real site; you give them the code; they replay it immediately. FIDO2 mitigates this with origin binding.
