...
## File Sharing and Sync Technologies (E2EE, P2P, and Ephemeral Links)

**Traditional file sharing** (Dropbox links, Google Drive shares, WeTransfer): files stored in plaintext on provider's servers. Provider can access, scan (for malware/CSAM), comply with legal requests. Convenient but no privacy.

**E2EE file sharing**: file encrypted on sender's device, uploaded to server as ciphertext. Recipient decrypts with shared key (embedded in link or separate password). Server cannot read file content. Examples: Send (Firefox Send fork), Bitwarden Send.

**Link-based sharing**: generate unique URL containing file ID + decryption key (often in fragment: `https://example.com/#key`). Fragment not sent to server (client-side only). Anyone with link can decrypt. Optional password adds second layer.

**Ephemeral/time-limited**: file auto-deletes after N downloads or time period. Reduces exposure window. Requires trust that provider actually deletes.

**P2P file sync** (Syncthing): no central server. Devices connect directly (local network) or via relay servers (encrypted transit). Block Exchange Protocol: files split into blocks, synchronized incrementally. Each device sees plaintext but no third party involved.

**Onion-routed sharing** (OnionShare): sender runs temporary Tor hidden service. Shares .onion URL. Recipient connects via Tor; neither party's IP exposed. Files never leave sender's device until downloaded. Strongest anonymity but requires both parties use Tor.

**Example: Send (encrypted ephemeral sharing)**:
1. Upload file to Send instance.
2. Client generates random key, encrypts file with AES-256-GCM.
3. Uploads ciphertext to server.
4. Returns link: `https://send.vis.ee/#<file_id>#<key>`. Key in URL fragment (not sent to server).
5. Recipient opens link, client extracts key from fragment, downloads ciphertext, decrypts locally.
6. File auto-deletes after 1 download or 24 hours (configurable).

**Syncthing architecture**: 
- Each device has unique device ID (derived from public key).
- Devices exchange signed announcements (via global discovery servers or local broadcast).
- Establish encrypted connection (TLS with mutual authentication).
- Synchronize folder states: compare block hashes, request missing/changed blocks.
- All data encrypted in transit (TLS). No E2EE at rest (files stored plaintext on each device).
