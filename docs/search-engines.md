---
meta_title: "Recommended Search Engines: Anonymous Alternatives to Google - The Cloud Sucks"
...
## Search Engine Privacy (Queries, Indexes, and Tracking)

**What search engines see**: your query, IP address, User-Agent (browser/OS), timestamp, referrer. Logged data can build profile: interests, location, identity (if logged in or cross-referenced with other data).

**Index sources**: 
- **Independent index** (Brave, Google, Bing): crawls web, builds own database. Full control; potential for bias or censorship at index level.
- **Meta-search** (SearXNG, Startpage): queries other engines, aggregates results. No index maintenance; dependent on upstream; less data linkage (queries come from proxy IP, not yours).

**Search query leakage**: queries sent in URL (GET requests). If you click a result, **referrer header** exposes your query to destination site (unless `Referrer-Policy: no-referrer` or HTTPS → HTTP). Private browsers strip referrer; many search engines use redirect/tracking URLs to hide queries (and track clicks).

**Tracking mechanisms**: 
- **Account-based**: Google/Bing with login; full history linked to identity.
- **IP-based**: no login but ISP assigns semi-static IP; queries over time build profile.
- **Cookie/fingerprint**: persistent identifiers track across sessions even without account.

**Anonymous usage**: 
1. Use search engine with no-logging policy (Brave Search, DuckDuckGo).
2. Use Tor Browser or VPN to hide IP from search provider.
3. Disable JavaScript if using meta-search (SearXNG) to reduce fingerprinting.
4. Avoid clicking ads (often include tracking tokens).

**Tor hidden services**: Brave, DuckDuckGo, Startpage offer .onion addresses. Entire connection over Tor; search provider doesn't see your real IP. Stronger anonymity but slower.

**Example: Google vs private search**:
- **Google logged in**: query stored with account, used for ad targeting across services (Gmail, YouTube, Maps). Profile sold/shared with advertisers.
- **DuckDuckGo**: query processed, results returned, no logs retained (per policy). No persistent identifier linking your searches over time.
...
Startpage's majority shareholder is System1 who is an adtech company. We don't believe that to be an issue as they have a distinctly separate [privacy policy](https://system1.com/terms/privacy-policy). The The Cloud Sucks team reached out to Startpage [back in 2020](https://blog.thecloudsucks.guide/2020/05/03/relisting-startpage) to clear up any concerns with System1's sizeable investment into the service, and we were satisfied with the answers we received.
