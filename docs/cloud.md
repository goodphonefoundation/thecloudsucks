---
meta_title: "The Best Private and Secure Cloud Storage Providers - The Cloud Sucks"
title: Cloud Storage
...
## Cloud Storage and E2EE (Client-Side vs Server-Side Encryption)

**Server-side encryption (SSE)**: provider encrypts files on their servers with keys they control (Dropbox, Google Drive by default). Protects against disk theft but provider can decrypt on demand (for deduplication, indexing, legal requests). You must trust provider not to access files.

**Client-side encryption (E2EE)**: files encrypted on your device before upload. Provider stores ciphertext; only you have decryption key. Provider cannot read files even if compromised or compelled.

**E2EE implementation**:
1. Generate encryption key derived from passphrase (KDF: Argon2, PBKDF2).
2. Encrypt file with symmetric key (AES-256-GCM, ChaCha20-Poly1305).
3. Upload ciphertext to cloud.
4. Store key locally or derive from passphrase on each use (zero-knowledge).

**Metadata leakage**: even with E2EE, provider sees file sizes, upload times, access patterns, folder structure (unless using obfuscation like Peergos/IPFS). Filename encryption required to hide content clues.

**Key management challenges**: 
- **User-managed keys**: lost passphrase = permanent data loss. No recovery.
- **Recovery keys**: backup key held by provider or printed by user. Reduces security if provider has recovery key.
- **Shared files**: need to encrypt file key with each recipient's public key (hybrid encryption: RSA/Curve25519 for key exchange, AES for file).

**Deduplication and E2EE**: SSE allows server-side dedup (one copy of popular file stored). E2EE breaks this: each user's encrypted copy is unique ciphertext. Some providers use convergent encryption (derive key from file hash) to allow dedup but leaks information (same plaintext → same ciphertext).

**Example: Proton Drive E2EE**:
1. User creates account, derives master key from password (SRP + Argon2).
2. Upload file: client generates random file key, encrypts file with AES-256-GCM.
3. File key encrypted with user's public key (asymmetric).
4. Encrypted file + encrypted file key uploaded to server.
5. Server stores ciphertext; cannot decrypt (lacks private key).
6. Recipient: decrypt file key with private key, decrypt file with file key.

**Cryptomator approach**: create encrypted vault on any cloud provider. Vault contents encrypted before sync client uploads. Provider sees random files in vault folder; you decrypt locally. Works with Dropbox, Google Drive, OneDrive, etc.

<details class="admonition info" markdown>
<summary>Looking for Nextcloud?</summary>

For more technical readers, Nextcloud is [still a recommended tool](self-hosting/file-management.md#nextcloud) for self-hosting a file management suite, however we do not recommend third-party Nextcloud storage providers at the moment, because we do [not recommend](https://discuss.thecloudsucks.guide/t/dont-recommend-nextcloud-e2ee/10352/29) Nextcloud's built-in E2EE functionality for home users.
