...
When you visit a website, a numerical address is returned. For example, when you visit `thecloudsucks.guide`, the address `192.98.54.105` is returned.
...
2. We can then use [`dig`](https://en.wikipedia.org/wiki/Dig_(command)) (Linux, macOS, etc.) or [`nslookup`](https://en.wikipedia.org/wiki/Nslookup) (Windows) to send the DNS lookup to both servers. Software such as web browsers do these lookups automatically, unless they are configured to use encrypted DNS.
    ...
    === "Linux, macOS"
        ...
        ```
        dig +noall +answer thecloudsucks.guide @1.1.1.1
        dig +noall +answer thecloudsucks.guide @8.8.8.8
        ```
    === "Windows"

        ```
        nslookup thecloudsucks.guide 1.1.1.1
        nslookup thecloudsucks.guide 8.8.8.8
        ```
...
|-----|----------|-----------|-------------|----------|--------|------------------------------------------------------------------------|
| 1   | 0.000000 | 192.0.2.1 | 1.1.1.1     | DNS      | 104    | Standard query 0x58ba A thecloudsucks.guide OPT                          |
| 2   | 0.293395 | 1.1.1.1   | 192.0.2.1   | DNS      | 108    | Standard query response 0x58ba A thecloudsucks.guide A 198.98.54.105 OPT |
| 3   | 1.682109 | 192.0.2.1 | 8.8.8.8     | DNS      | 104    | Standard query 0xf1a9 A thecloudsucks.guide OPT                          |
| 4   | 2.154698 | 8.8.8.8   | 192.0.2.1   | DNS      | 108    | Standard query response 0xf1a9 A thecloudsucks.guide A 198.98.54.105 OPT |
...
2. Second, make a request with `curl`:
    ...
    ```bash
    curl -vI --doh-url https://1.1.1.1/dns-query https://thecloudsucks.guide
    ```
...
The simplest way to determine browsing activity might be to look at the IP addresses your devices are accessing. For example, if the observer knows that `thecloudsucks.guide` is at `198.98.54.105`, and your device is requesting data from `198.98.54.105`, there is a good chance you're visiting The Cloud Sucks.
...
2. Then we visit [https://thecloudsucks.guide](https://thecloudsucks.guide).
...
4. Next we want to analyze the results:
    ...
    We will see the connection establishment, followed by the TLS handshake for the The Cloud Sucks website. Around frame 5. you'll see a "Client Hello".
...
1. Get the server certificate and use [`sed`](https://en.wikipedia.org/wiki/Sed) to keep just the important part and write it out to a file:
    ...
    ```bash
    openssl s_client -connect thecloudsucks.guide:443 < /dev/null 2>&1 |
...
2. Get the intermediate certificate. [Certificate Authorities (CA)](https://en.wikipedia.org/wiki/Certificate_authority) normally don't sign a certificate directly; they use what is known as an "intermediate" certificate.
    ...
    ```bash
    openssl s_client -showcerts -connect thecloudsucks.guide:443 < /dev/null 2>&1 |
...
DNSSEC implements a hierarchical digital signing policy across all layers of DNS. For example, in the case of a `thecloudsucks.guide` lookup, a root DNS server would sign a key for the `.org` nameserver, and the `.org` nameserver would then sign a key for `thecloudsucks.guide`’s authoritative nameserver.

<small>Adapted from [DNS Security Extensions (DNSSEC) overview](https://cloud.google.com/dns/docs/dnssec) by Google and [DNSSEC: An Introduction](https://blog.cloudflare.com/dnssec-an-introduction) by Cloudflare, both licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0).</small>

## What is QNAME minimization?

A QNAME is a "qualified name", for example `discuss.thecloudsucks.guide`. In the past, when resolving a domain name your DNS resolver would ask every server in the chain to provide any information it has about your full query. In this example below, your request to find the IP address for `discuss.thecloudsucks.guide` gets asked of every DNS server provider:

| Server                 | Question Asked                              | Response                                    |
|------------------------|---------------------------------------------|---------------------------------------------|
| Root server            | What's the IP of discuss.thecloudsucks.guide? | I don't know, ask .net's server...          |
| .net's server          | What's the IP of discuss.thecloudsucks.guide? | I don't know, ask The Cloud Sucks' server... |
| The Cloud Sucks' server | What's the IP of discuss.thecloudsucks.guide? | 5.161.195.190!                              |
...
| .net's server          | What's the nameserver for thecloudsucks.guide?         | *Provides The Cloud Sucks' server* |
| The Cloud Sucks' server | What's the nameserver for discuss.thecloudsucks.guide? | This server!                      |
| The Cloud Sucks' server | What's the IP of discuss.thecloudsucks.guide?          | 5.161.195.190                     |
