...
## Tor vs I2P vs Other Overlay Networks

**Overlay networks**: build anonymity network on top of regular internet. Encrypt and route traffic through multiple hops to hide origin/destination. Different architectures, threat models, use cases.

**Tor architecture**: 3-hop circuits (guard, middle, exit). Exit nodes connect to clearnet. Hidden services (.onion) use rendezvous protocol (no exit node). ~10k relays. Centralized directory authorities (hardcoded, trusted). Pluggable transports (obfs4, Snowflake) for censorship circumvention.

**I2P architecture**: garlic routing (multiple messages bundled in single encrypted "garlic"). All traffic stays within I2P network (no exit nodes by default; outproxies exist but discouraged). ~50k routers. Every node participates in routing (distributed load). Eepsites (.i2p) use one-way tunnels (inbound + outbound separately).

**Tor advantages**: 
- Clearnet access (browse normal websites anonymously).
- Tor Browser (hardened Firefox, anti-fingerprinting, widely used = good anonymity set).
- More academic research, security audits, funded development.
- Better censorship resistance (pluggable transports, bridge infrastructure).

**I2P advantages**: 
- P2P-friendly (BitTorrent, file sharing; Tor discourages/bans this).
- Better hidden service performance (all traffic internal, optimized for eepsites).
- Distributed network (no centralized directory authorities to compromise or censor).
- Every user contributes bandwidth (vs Tor's reliance on volunteers).

**Snowflake (Tor pluggable transport)**: WebRTC-based proxy. Users in uncensored regions run Snowflake in browser; acts as bridge for censored users. Censor can't easily block (proxies change frequently, use WebRTC = looks like video call). Low-bandwidth (not full Tor relay). Helps Tor network scale in censored environments.

**Orbot (Tor for Android/iOS)**: system-wide VPN mode routes all apps through Tor (or per-app proxy). Limitations: iOS memory limits cause crashes; no effective kill switch on iOS; mobile apps not designed for Tor (can leak). Desktop Tor Browser still safer.

**Example: Tor vs I2P for torrenting**:
- **Tor**: exit nodes see your torrent traffic, many exit relays block BitTorrent. Slows network, risks de-anonymization (BitTorrent leaks IP in various ways). Tor Project discourages this.
- **I2P**: built-in BitTorrent clients (I2PSnark, integrated torrenting). Traffic stays internal to I2P network. Swarm = only I2P users. Slower than clearnet but private, supported use case.
...
You can try connecting to *The Cloud Sucks* via Tor at [xoe4vn5uwdztif6goazfbmogh6wh5jc4up35bqdflu6bkdc5cas5vjqd.onion](http://www.xoe4vn5uwdztif6goazfbmogh6wh5jc4up35bqdflu6bkdc5cas5vjqd.onion).
...
You can try connecting to *The Cloud Sucks* via I2P at [privacyguides.i2p](http://privacyguides.i2p/?i2paddresshelper=fvbkmooriuqgssrjvbxu7nrwms5zyhf34r3uuppoakwwsm7ysv6q.b32.i2p).
