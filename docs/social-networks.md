...
## Federated Social Networks (ActivityPub, Matrix, Decentralization)

**Centralized social media problems**: Facebook, Twitter, Instagram = single company controls: data (full access to messages/posts/metadata), moderation (arbitrary bans, no appeal), algorithms (engagement optimization = outrage), monetization (ads, data sales). No interoperability: can't message Twitter from Facebook.

**Federation**: like email. Multiple servers interoperate using open protocol. You pick server; server federates with others. Example: alice@mastodon.social can follow bob@tech.instance. Servers exchange posts via standard protocol (ActivityPub for Mastodon, Matrix for Element).

**ActivityPub** (Mastodon, PeerTube, Pixelfed): W3C standard for social networking. Client-to-server (post, follow) + server-to-server (federation). JSON-LD messages. Servers announce posts to followers' servers. Decentralized social graph (each server tracks own users' follows).

**Matrix protocol**: real-time communication. Rooms hosted across multiple homeservers (federated state). End-to-end encryption optional (Double Ratchet + Megolm for group chats). Designed for chat but supports social features (spaces, threads).

**Privacy implications of federation**:
- **Server sees your activity**: your homeserver/instance has full access to your posts (even followers-only), DMs (unless E2EE), metadata (who you follow, post times, IP address). Choose trusted admin or self-host.
- **Federation leaks metadata**: when you follow someone on another server, their server learns you exist. Public posts replicated across servers (deletion requests may be ignored). Defederation = censorship (server blocks others; can't see their content).
- **DMs not private**: Mastodon DMs are just posts with limited visibility. **Not E2EE**. Admin of your server + recipient's server can read. Matrix DMs can be E2EE but verify keys.

**Decentralization vs censorship**: no single point of control = harder to censor network-wide. But: individual servers can ban you, defederate (isolate you), or censor content. Moving servers possible but lose post history/followers (depending on protocol). Self-hosting = maximum control but requires technical skill, costs money.

**Metadata exposure**: social graph (who you follow/interact with), posting patterns (times, frequency), content metadata (hashtags, media). Public by default in many cases. Mastodon can hide follows/followers; still visible to server admin. Automated post deletion reduces historical profile building.

**Example: Mastodon federation flow**:
1. Alice (alice@masto.example) follows Bob (bob@social.example).
2. masto.example sends ActivityPub Follow request to social.example.
3. Bob's server approves (if public profile) or prompts Bob (if private).
4. Bob posts: "Hello world".
5. social.example sends Create activity with post to masto.example (and all followers' servers).
6. masto.example stores post in Alice's timeline.
7. Alice sees post in her feed.

Both servers have full copy of public post. If Bob's server goes offline, Alice still sees cached copy.
