---
meta_title: "Encrypted Private Email Recommendations - The Cloud Sucks"
...
## How Email Actually Works (Technical but Readable)

Modern email is a **store-and-forward** system built from several moving parts:

- **MUA (Mail User Agent)** – your email client (Thunderbird, FairEmail, Apple Mail, webmail).
- **MSA (Mail Submission Agent)** – the server that accepts your outgoing mail (usually on TCP 587 or 465).
- **MTA (Mail Transfer Agent)** – servers that relay mail between domains (usually on TCP 25).
- **MDA (Mail Delivery Agent)** – the component that finally writes the message into the recipient's mailbox.
- **DNS + MX records** – tell the sending server *which* host is responsible for accepting mail for a domain.

### Example: alice@a.example → bob@b.example

```text path=null start=null
1. Alice's client → A's submission server (MSA)
   - Protocol: SMTP submission (TCP 587) with STARTTLS

2. A's MTA → B's MTA
   - A looks up MX for b.example via DNS
   - Connects to B over SMTP (TCP 25), ideally with STARTTLS

3. B's MDA stores the message in Bob's mailbox

4. Bob's client reads mail from B
   - Protocol: IMAP (TCP 993 with TLS) or POP3 (TCP 995 with TLS) or via webmail
```

At each hop:

- The **transport channel** can be encrypted (TLS for SMTP/IMAP/POP3/HTTPS).
- The **message itself** (headers + body) is usually *not* end-to-end encrypted unless you use OpenPGP or S/MIME.

### What is Visible to Whom?

Even with TLS everywhere, different parties still see different parts of the message:

- **Your device / client** – sees everything in plaintext (obviously).
- **Your provider (sending side)** – sees full message and metadata.
- **Recipient's provider** – sees full message and metadata.
- **Intermediate MTAs** (if any) – generally see full message and metadata.

The **email metadata** – envelope sender/recipient, `From`, `To`, `Date`, `Subject`, and `Received` headers – is visible to every server that relays the message, even when you use OpenPGP for the body.

A simplified raw message might look like this:

```text path=null start=null
Received: from client-ip.example (client-ip.example [203.0.113.10])
        by mail.a.example with ESMTPSA id 12345
        for <bob@b.example>; Mon, 10 Nov 2025 10:00:00 +0000
Received: from mail.a.example (mail.a.example [198.51.100.20])
        by mail.b.example with ESMTPS id 67890
        for <bob@b.example>; Mon, 10 Nov 2025 10:00:02 +0000
From: Alice <alice@a.example>
To: Bob <bob@b.example>
Subject: Meeting notes
Date: Mon, 10 Nov 2025 10:00:00 +0000
Message-ID: <abc123@a.example>

[body starts here – optionally OpenPGP‑encrypted]
```

End-to-end encryption (OpenPGP/S/MIME) typically **only protects the body and some headers**, not the full routing information.

### Why Email is Hard to Make Private

Compared to modern messengers, email has structural limitations:

- **Long-lived storage** – messages are kept for years or decades on servers and backups.
- **Metadata-heavy** – routing information and subject lines leak a lot about your communications.
- **Multi-hop delivery** – multiple independent servers can see your messages.
- **Spam filtering and abuse detection** – providers often analyze content or patterns to fight spam and phishing.

That is why The Cloud Sucks recommends **messengers with forward secrecy** for person-to-person chat, and **carefully chosen email providers** for account registrations, notifications, and slower, less sensitive communication.

### Authentication, Deliverability, and Anti-Abuse (SPF, DKIM, DMARC, MTA‑STS, DANE)

Serious email providers implement a stack of DNS-based mechanisms to make spoofing harder and transport more robust:

- **SPF (Sender Policy Framework – RFC 7208)**
  - A TXT record that says which IPs/hosts are allowed to send mail for a domain.
  - Receiving servers check SPF to detect forged `MAIL FROM`/`Return‑Path` domains.

- **DKIM (DomainKeys Identified Mail – RFC 6376)**
  - Adds a cryptographic signature to headers and body using a private key.
  - Public key is published in DNS; recipients verify that the domain really signed the message.

- **DMARC (RFC 7489)**
  - Tells receivers how to treat mail that fails SPF/DKIM and enforces *alignment* (From domain must align with SPF/DKIM domains).
  - Example policy: `v=DMARC1; p=reject; rua=mailto:dmarc-reports@example.com`.

- **MTA‑STS (RFC 8461) and TLS‑RPT (RFC 8460)**
  - MTA‑STS lets a domain declare that its MX servers must be reached with TLS and with valid certificates.
  - TLS‑RPT provides a reporting mechanism when TLS delivery fails.

- **DANE for SMTP (RFC 7672, via DNSSEC)**
  - Uses DNSSEC-signed TLSA records to bind specific certificates/keys to MX hosts.
  - Allows strict TLS enforcement without depending on the public CA ecosystem.

These are why our **criteria section** later on insists on valid SPF, DKIM, DMARC, MTA‑STS, TLS‑RPT, DANE, and DNSSEC – they are not just buzzwords, but critical for preventing spoofing, downgrade attacks, and delivery failures.

### Concrete Usage Examples

1. **Using a privacy-respecting provider only for registrations**
   - Create `you@yourdomain.example` with one of the recommended providers.
   - Use it to sign up for services (banks, social networks, utilities).
   - Combine with an [email aliasing service](email-aliasing.md) so each site gets a unique address (e.g., `github@yourdomain.example`, `bank@yourdomain.example`).

2. **Receiving OpenPGP-encrypted notifications from services**
   - Publish your OpenPGP public key via WKD using your provider or your own domain.
   - Some services (bug trackers, security mailing lists, etc.) can automatically encrypt notifications to your key.

3. **Separating "chat" from "records"**
   - Use a messenger like Signal or Matrix for conversational back-and-forth.
   - Use email only for things you reasonably expect to be stored and referenced later (invoices, account changes, legal notices).

**Selected technical references:**

- SMTP and message format – RFC 5321, RFC 5322
- SPF – RFC 7208; DKIM – RFC 6376; DMARC – RFC 7489
- MTA‑STS – RFC 8461; TLS‑RPT – RFC 8460; DANE for SMTP – RFC 7672
