---
meta_title: "Private VPN Service Recommendations and Comparison, No Sponsors or Ads - The Cloud Sucks"
title: VPN Services
icon: material/vpn
...
## How a VPN Actually Works (Technical but Practical)

At a protocol level, a VPN client creates an *encrypted tunnel* between your device and a VPN server. All traffic that is routed into this tunnel is:

- **Encrypted on your device** (before it leaves your network interface).
- **Encapsulated** in a VPN packet (WireGuard, OpenVPN, IKEv2, etc.).
- **Transported over the Internet** to the VPN server.
- **Decrypted at the VPN server**, which then sends the traffic to the final destination (websites, APIs, etc.).

```text path=null start=null
[Your device] --encrypted tunnel--> [VPN server] --plain Internet--> [Website]
        ^                                           ^
        |                                           |
   ISP / Wi‑Fi                                  Website sees
   only sees:                                   your VPN IP
   "you ↔ VPN"                                  not your home IP
```

Different VPN protocols implement this tunnel differently:

- **OpenVPN** (RFC 7281 family)
  - Runs over TCP or UDP.
  - Uses TLS for key exchange (similar to HTTPS).
  - Typically uses AES‑256‑GCM/ChaCha20‑Poly1305 for data encryption.
- **WireGuard**
  - Runs over UDP only.
  - Uses a fixed modern crypto suite (Curve25519, ChaCha20‑Poly1305, BLAKE2, HKDF).
  - Maintains a very small codebase (~4–5k LOC in kernel) which improves reviewability.
- **IKEv2/IPsec** (RFC 7296)
  - Common on mobile OSes (iOS, Android) as a built‑in protocol.
  - Uses IPsec for data encapsulation and IKEv2 for key management.

In all cases, the **outer IP header** on the Internet only shows:

- *Source*: your IP address.
- *Destination*: the VPN server's IP address.

The **inner IP header and payload** (the actual sites you visit, TCP ports, etc.) are encrypted inside the tunnel.

### What a VPN Changes in Practice

**Without VPN:**

- Your ISP / local network sees: `your IP → example.com:443` (plus DNS queries).
- Websites see: your *real* IP (e.g., home or mobile carrier).

**With VPN:**

- Your ISP / local network sees: `your IP → vpn.example:51820` (or OpenVPN/IKEv2 port).
- VPN provider sees: `your public IP` and all destinations you access *inside the tunnel* (unless you're using an additional layer like Tor).
- Websites see: the *VPN server's IP*, not your home IP.

This is why **trust** in the VPN provider is critical: you are effectively moving visibility from your ISP/Wi‑Fi operator to the VPN operator.

### Concrete Examples

1. **Public Wi‑Fi at an airport**
   - *Threat*: The Wi‑Fi operator or nearby attackers can sniff unencrypted traffic, perform ARP spoofing, or hijack DNS.
   - *With VPN*: All traffic inside the tunnel is encrypted, so passive sniffing reveals only that you are connected to a VPN server (plus some metadata like packet size and timing).

2. **Your ISP selling browsing data**
   - *Threat*: Many ISPs log and monetize DNS queries and destination IPs.
   - *With VPN*: The ISP only sees you talking to the VPN. DNS queries and destination IPs are visible to the VPN instead.

3. **Geo‑restricted content**
   - *Threat*: Services restrict access based on source IP country.
   - *With VPN*: Since websites see the VPN server IP, choosing a VPN server in a specific country can make you appear to be there. Some services actively block known VPN exit IPs.

4. **Torrenting on a hostile ISP**
   - *Threat*: ISP throttling or legal threats based on detected BitTorrent traffic.
   - *With VPN*: ISP sees only encrypted VPN traffic. The torrent swarm and trackers see the VPN IP, not your home IP. If your VPN has a kill switch and no leaks, disconnects won't reveal your real IP.

### Common Leak Vectors (and Why Criteria Matter)

A VPN can still leak information if **implementation or configuration** is bad. Typical leaks:

- **DNS leaks** – Your system continues to use your ISP's DNS instead of the VPN's or a secure resolver (DoT/DoH). Good clients force all DNS over the tunnel.
- **IPv6 leaks** – Some providers only tunnel IPv4. If IPv6 isn't disabled or tunneled, your real IPv6 address may reach sites directly.
- **WebRTC leaks** – Browsers can reveal local and public IPs via WebRTC unless blocked by browser settings or extensions.
- **Kill switch failures** – If the VPN drops and there is no proper firewall‑level kill switch, traffic may fall back to your regular connection.

The technical criteria later on this page (WireGuard support, IPv6 handling, port forwarding, audits, etc.) are designed specifically to reduce these risks.

**Selected technical references:**

- WireGuard protocol design – https://www.wireguard.com/protocol/
...
Proton VPN, in addition to accepting credit/debit cards, PayPal, and [Bitcoin](advanced/payments.md#other-coins-bitcoin-ethereum-etc), also accepts **cash/local currency** as an anonymous form of payment. You can also use [**Monero**](cryptocurrency.md#monero) to purchase vouchers for Proton VPN Plus and Proton Unlimited via their [official](https://discuss.thecloudsucks.guide/t/add-monero-as-an-anonymous-payment-method-for-proton-services/31058/15) reseller [ProxyStore](https://dys2p.com/en/2025-09-09-proton.html).
