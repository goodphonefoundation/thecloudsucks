...
## RSS/Atom Feeds and Privacy (Pull vs Push, Tracking)

**RSS/Atom fundamentals**: XML-based syndication formats. Sites publish feed URL (e.g., `example.com/feed.xml`). Your client polls feed periodically, fetches new items. Decentralized: no intermediary required.

**Privacy advantage of RSS**: you pull content from publisher. No account required (usually). Publisher sees your IP + User-Agent when you fetch feed but can't track you across sites (no cookies, no ad network pixels). Compare to browsing site directly (tracking scripts, ads) or using algorithm-driven feed (platform sees everything).

**Self-hosted vs cloud aggregators**: 
- **Local client** (Akregator, Newsboat, NetNewsWire): feeds fetched from your IP. Publisher knows someone at your IP reads their feed. No third-party tracking.
- **Cloud service** (Feedly, Inoreader): aggregator's servers fetch feeds on your behalf. Publisher sees aggregator's IP, not yours. But aggregator tracks your reading habits, can monetize (ads, analytics).
- **Self-hosted** (Miniflux, Nextcloud News): you control server. Fetches feeds from your server's IP. No third-party; still some metadata exposure to publishers.

**Feed tracking mechanisms**: some feeds include unique IDs per subscriber (in feed URL or item links). Publisher correlates fetches with subscriber. Avoid feeds with `?user=<id>` patterns.

**Atom vs RSS**: Atom (IETF standard, RFC 4287) is more rigorous spec than RSS (various versions). Both functionally similar. JSON Feed is newer, JSON-based alternative.

**WebSub (PubSubHubbub)**: push-based extension. Publisher notifies hub when new content available; hub pushes to subscribers (near real-time). Reduces polling but requires hub trust.

**Example: RSS feed fetch**:
1. Newsboat fetches `https://blog.example.com/feed.xml` every 30 minutes.
2. HTTP GET request from your IP. blog.example.com sees: IP, User-Agent, timestamp.
3. Server returns XML with recent posts (title, link, summary).
4. Newsboat parses, displays new items.
5. No cookies, no JavaScript, no tracking beyond HTTP logs.

**YouTube/Reddit RSS**: platforms offer RSS despite pushing algorithm-driven feeds. Use RSS to avoid login, tracking, recommendation bubble. Reddit: append `.rss` to subreddit URL. YouTube: use `feeds/videos.xml?channel_id=<ID>` (see documentation below).
