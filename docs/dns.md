...
## DNS Fundamentals (and What Encrypted DNS Does and Doesn't Protect)

**DNS resolution flow**: `browser → stub resolver → recursive resolver → authoritative nameservers`. Your ISP's recursive resolver sees every domain you query (and your IP). Authoritative nameservers see the query but typically only the resolver's IP (unless EDNS Client Subnet is used).

**Unencrypted DNS** (UDP port 53): queries and responses in plaintext. ISP, network observers, and anyone on-path see every domain you look up. Trivial to log, block, or tamper with (DNS hijacking).

**Encrypted DNS protocols**:
- **DoT (DNS-over-TLS, RFC 7858, port 853)**: TLS tunnel for DNS. Hides queries from network observers; ISP sees you're connecting to a resolver but not which domains.
- **DoH (DNS-over-HTTPS, RFC 8484)**: DNS queries inside HTTPS (port 443). Looks like web traffic; harder to block than DoT but same privacy properties.
- **DoQ (DNS-over-QUIC, RFC 9250)**: DNS over QUIC (UDP-based, low latency, multiplexed). Emerging standard.
- **DNSCrypt**: predates DoT/DoH; similar properties (authenticated encryption, uses own protocol). Less widely deployed.

**What encrypted DNS protects**: queries from your device to your chosen resolver. ISP can't see domain names, can't hijack responses (authentication).

**What encrypted DNS does NOT protect**:
- **SNI (Server Name Indication)**: TLS ClientHello includes domain in plaintext (most sites). Encrypted Client Hello (ECH, draft standard) fixes this.
- **IP addresses**: after DNS resolution, you connect to the IP. ISP sees destination IP; often possible to infer domain from IP (reverse DNS, IP-domain databases).
- **Traffic analysis**: timing, packet sizes, and patterns can reveal which sites you visit.
- **Resolver trust**: your encrypted DNS provider sees all your queries. Choose carefully.

**DNSSEC** (RFC 4033): cryptographic signatures on DNS records. Prevents tampering but doesn't encrypt queries. DoT/DoH + DNSSEC is best practice.

**ECS (EDNS Client Subnet, RFC 7871)**: resolver sends part of your IP to authoritative nameserver (for CDN geolocation). Privacy-conscious resolvers anonymize ECS or disable it.

**Example: ISP blocking via DNS**:
1. You query `blocked-site.example` via ISP resolver.
2. ISP returns fake IP (NXDOMAIN or redirect to block page).
3. Switch to encrypted DNS (DoH to Cloudflare): ISP can't see query, can't tamper with response.
4. You get real IP, connect successfully.

ISP can still block by IP, but DNS blocking is easiest and most common method.
