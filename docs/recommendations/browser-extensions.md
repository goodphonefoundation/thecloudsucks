...
## Browser Extensions (Permissions, Attack Surface, and Content Blocking)

**Extension privileges**: extensions can request broad permissions: "read and change all your data on all websites" (see every page you visit, modify content, steal credentials/cookies), "access tabs" (track browsing history), "access clipboard" (steal copied data). Malicious/compromised extensions are powerful attack vectors.

**Manifest V2 vs V3**: 
- **MV2** (legacy): extensions run persistent background scripts with full webRequest API. Can block/modify network requests before they're sent. Powerful but higher resource usage, security risk (arbitrary code execution).
- **MV3** (current): declarative rules, no arbitrary code in background. More restrictive (Google's stated goal: security, performance). Content blockers like uBlock Origin lose dynamic filtering features. uBlock Origin Lite uses declarativeNetRequest API (limited rule count, less flexible).

**Fingerprinting via extensions**: websites can detect installed extensions (check for injected CSS/JS, enumerate accessible web_accessible_resources). Unique extension combination = unique fingerprint. Tor Browser forbids extensions for this reason (plus attack surface).

**Site isolation weakness**: extensions can access content across origin boundaries (bypass same-origin policy). Compromised extension can exfiltrate data from sensitive sites (bank, email) you visit.

**Content blocking mechanisms**: 
- **Network-level blocking**: intercept HTTP requests, block before connection made (uBlock Origin MV2: webRequest API). Fast, prevents resource loading entirely.
- **Cosmetic filtering**: hide elements via CSS after page loads (uBlock Origin: `##.ad-container`). Doesn't prevent loading; just hides visually.
- **Scriptlet injection**: inject JavaScript to interfere with site scripts (anti-adblock bypass, cookie consent auto-dismiss).

**Filter lists**: subscription-based rulesets (EasyList, EasyPrivacy, uBlock filters). Updated regularly. Too many lists = slower browsing, false positives. Stick to reputable, maintained lists.

**Example: uBlock Origin blocking an ad**:
1. Page loads, requests `https://ads.example.com/banner.js`.
2. uBlock Origin intercepts webRequest before fetch.
3. Checks filter lists: matches rule `||ads.example.com^`.
4. Blocks request; returns network error to page.
5. Ad never loads, no tracking pixel fires.

With MV3 (uBlock Origin Lite), rules compiled into declarativeNetRequest. Browser blocks at network stack level (before extension sees request). Faster, more secure, but less flexible (can't dynamically update rules based on page context).
