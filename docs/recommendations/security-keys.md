...
## How FIDO2/WebAuthn Works (and Why It's Phishing-Resistant)

**Key generation**: when you register a security key with a website, the key generates a public/private keypair specific to that site's origin (e.g., `example.com`). Private key never leaves the hardware module.

**Authentication ceremony**: site challenges your key with a nonce and its origin. Key signs the challenge with the private key only if the origin matches. Browser enforces origin binding; even a phishing site at `examp1e.com` can't trick the key.

**Attestation**: during registration, key can optionally prove its make/model via a manufacturer signature. Sites can require specific key types (e.g., FIPS-certified).

**Credential storage**: keys store multiple credentials (credential ID + private key). Authenticator retrieves the right credential using the relying party ID (RP ID, derived from domain).

**Platform vs roaming authenticators**: platform authenticators are built into devices (Touch ID, Windows Hello). Roaming authenticators are external (USB, NFC). FIDO2 supports both.

**Resident keys (discoverable credentials)**: credential stored on the key (limited slots). Allows passwordless login: key presents username candidates. Non-resident keys require username first, then server sends credential ID.

**User verification**: PIN or biometric on the key itself ensures physical possession isn't enough (2FA built into one device). UV can be required by relying party.

**Example: phishing attack blocked**:
1. Attacker sends you link to `examp1e.com` (fake).
2. You enter credentials; attacker proxies to real `example.com`, gets session.
3. You try to use security key on fake site.
4. Key checks origin, sees `examp1e.com`, refuses to sign (no matching credential).
5. Attack fails. TOTP would have been replayed successfully.

**Why hardware keys beat software**: tamper-resistant secure element protects private key from extraction even if host is compromised. Software authenticators (passkeys in password managers) are easier to use but rely on OS/app security.
