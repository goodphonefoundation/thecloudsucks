---
meta_title: "The Best Private Instant Messengers - The Cloud Sucks"
...
[:material-movie-open-play-outline: Video: It's time to stop using SMS](https://www.thecloudsucks.guide/videos/2025/01/24/its-time-to-stop-using-sms-heres-why){ .md-button }

## Secure Messaging: Signal Protocol and Double Ratchet

**Signal Protocol** (used by Signal, WhatsApp, many others): combines X3DH (Extended Triple Diffie-Hellman) key agreement + Double Ratchet encryption. Each message has a unique key; compromise of one key doesn't expose past or future messages (forward secrecy + post-compromise security).

**Double Ratchet**: two layers. **Symmetric ratchet** (KDF chain): each message derives new keys from prior state. **DH ratchet**: periodic Diffie-Hellman exchanges refresh root keys. Together they provide forward secrecy (old keys deleted after use) and post-compromise security (attacker must remain present during every DH exchange).

**Why metadata matters**: E2EE protects message content but not metadata. Server sees who talks to whom, when, and message sizes. This is enough to infer social graphs and communication patterns.

**Sealed Sender** (Signal feature): sender identity encrypted for initial routing, only recipient can decrypt. Reduces metadata leakage but requires recipient to have added you to contacts.

**Centralized vs federated vs P2P**:
- **Centralized** (Signal): one organization runs all servers. Easier to secure and update; single point of failure/compromise.
- **Federated** (Matrix, XMPP): multiple servers interoperate. Decentralized trust; metadata visible to your server and recipient's server; weaker consistency for E2EE key distribution.
- **P2P** (Briar, SimpleX queues): no persistent servers, direct connections or relayed through untrusted nodes. Strongest metadata protection; harder to maintain availability.

**Example: Signal message flow**:
1. Alice and Bob complete X3DH key agreement (initial DH exchange + pre-keys).
2. Alice encrypts message M with current chain key K_n, derives K_{n+1}.
3. Every few messages, Alice and Bob perform DH exchange to refresh root key.
4. Server relays ciphertext; sees sender, recipient, timestamp, size. Cannot see content.

**SMS is unencrypted**: plaintext to your carrier, recipient's carrier, and any intermediate parties. No forward secrecy, no authentication. Vulnerable to SIM swaps and SS7 attacks.
...
[Signal Configuration and Hardening :material-arrow-right-drop-circle:](https://blog.thecloudsucks.guide/2022/07/07/signal-configuration-and-hardening)
...
**Molly** is an alternative Signal client for Android which allows you to encrypt the local database with a passphrase at rest, to have unused RAM data securely shredded, to route your connection via Tor, and [more](https://blog.thecloudsucks.guide/2022/07/07/signal-configuration-and-hardening#privacy-and-security-features). It also has usability improvements including scheduled backups, automatic locking, and the ability to use your Android phone as a linked device instead of the primary device for a Signal account.
