...
## Email Aliasing (Forwarding, Tracking Prevention, Plus Addressing)

**How aliasing works**: service generates unique email address (alias) for you. Incoming mail to alias is forwarded to your real mailbox. Outgoing replies rewritten to appear from alias. Service sees plaintext email (unless using PGP encryption).

**Architecture**: MX records point to aliasing service. Incoming: `mail → aliasing service → forward to real mailbox`. Outgoing: `your mailbox → aliasing service → rewrite From header → recipient`. Service acts as SMTP relay.

**Privacy advantages**: 
1. **Compartmentalization**: each site gets unique alias. Breach at site A doesn't expose real email or compromise site B. Can disable leaky alias individually.
2. **Tracking prevention**: advertisers can't correlate activity across sites via email address. IAB requires normalizing `user+tag@domain` → `user@domain` for tracking; true aliases don't normalize.
3. **Mailbox portability**: switch email providers without changing public addresses. Update forwarding destination; aliases unchanged.

**Plus addressing limitations** (`user+tag@example.com`): trivially stripped by recipients (everything after `+` removed). IAB guidelines require normalization. Identifies you as privacy-conscious (may trigger different treatment). Doesn't hide real mailbox domain.

**Catch-all vs dedicated aliasing**: 
- **Catch-all on custom domain**: any address at `@yourdomain.com` accepted. Simple but no per-alias control (can't disable individually). Unique domain fingerprints you.
- **Aliasing service**: per-alias enable/disable, reply tracking, stats. Shared domain blends you with other users. Can use custom domain if desired (less privacy but branding).

**PGP encryption option**: some services (Addy.io, SimpleLogin premium) auto-encrypt forwarded mail with your public key before delivery. Reduces trust: aliasing service still sees plaintext when forwarding, but mailbox provider sees only ciphertext (one less party to trust).

**Example: email flow with aliasing**:
1. Register at `example.com` with alias `shop123@addy.io` (generated by Addy.io).
2. Site sends email to `shop123@addy.io`.
3. Addy.io receives mail, forwards to your real address `user@proton.me`.
4. You reply: email leaves Proton as `user@proton.me`.
5. Addy.io rewrites From header to `shop123@addy.io`, sends to example.com.
6. example.com sees only `shop123@addy.io`; your real mailbox hidden.

If `shop123` alias starts receiving spam, disable it. Other aliases unaffected.
