...
## Mobile Spyware and Forensic Artifacts (Persistent vs Non-Persistent)

**Zero-click exploits**: attacker compromises device without user interaction. Delivered via iMessage, WhatsApp, SMS, or malicious websites. Modern sophistication: Pegasus (NSO Group), Predator (Intellexa). Exploit chain: CVE in messaging/browser → kernel exploit → persistence mechanism.

**Non-persistent malware**: lives in memory only. Reboot clears infection. Attacker must re-exploit after reboot. Common for zero-click (avoids forensic traces). Example: process injection, in-memory rootkit. Regular reboots force re-infection (attacker must maintain active access).

**Persistent malware**: survives reboot. Modifies system partition, installs malicious app, or uses OS persistence mechanisms. Harder to achieve on modern phones (verified boot, SELinux) but still possible via bootloader exploits or signed malicious system apps.

**Indicators of Compromise (IOCs)**: forensic artifacts left by malware. SMS messages with exploit payloads, suspicious processes in crash logs, network connections to C2 servers, modified system files. Public IOCs (published by Amnesty, Citizen Lab, vendors) used by MVT. Private IOCs (held by intelligence agencies, vendors) not detectable by public tools.

**iOS forensic sources**: iTunes backup (app data, SMS, call logs, Photos metadata, limited system logs), sysdiagnose logs (detailed crash reports, process listings, network connections), device logs (Analytics & Improvements section). MVT parses these for IOC matches.

**Android forensic limitations**: limited diagnostic info accessible without root. Most useful data in `/data/system` requires privileged access. MVT's Android capabilities weaker: can check for suspicious apps, packages, but can't deeply inspect system state.

**Hardware attestation** (Android Auditor): device's secure element (Titan M, Qualcomm SPU) signs attestation response proving OS integrity. Bootloader verifies boot chain, signs report. Remote verifier checks signature + certificate chain. Detects: bootloader unlock, modified system, rollback attacks. TOFU model: initial attestation establishes baseline; subsequent checks detect changes.

**Example: Pegasus detection via MVT**:
1. Create encrypted iTunes backup of iPhone.
2. Run `mvt-ios decrypt-backup` to extract files.
3. MVT scans SMS database for known exploit messages (image attachments with 0-byte size, specific sender patterns).
4. Checks Safari history for suspicious domains (NSO infrastructure).
5. Analyzes crash logs for processes crashing at known exploitation times.
6. Reports potential IOCs with threat attribution.

IOC match ≠ definitive proof. Requires expert analysis, timeline correlation, additional forensics.
...
**We are unable to help you directly beyond this.** We are happy to discuss your specific situation or circumstances and review your results in our [community](https://discuss.thecloudsucks.guide) spaces, but it is unlikely we can assist you beyond what is written on this page.
...
3. Enable [Lockdown Mode](https://blog.thecloudsucks.guide/2022/10/27/macos-ventura-privacy-security-updates/#lockdown-mode).
