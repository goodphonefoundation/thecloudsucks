...
## Privacy Frontends (Proxying, JavaScript, and Tracking)

**What frontends do**: fetch content from platform (YouTube, Reddit, TikTok) on your behalf, strip tracking/ads/JavaScript, serve clean HTML. You interact with frontend, not original site.

**Privacy advantage**: platform sees frontend server's IP (not yours, if using public instance). No cookies, no login, no tracking pixels. Often no JavaScript required (faster, fewer attack vectors).

**Self-hosting vs public instances**:
- **Public instance**: convenient but instance operator can log your activity. Choose reputable instances (check privacy policy, Tor support). Traffic analysis risk: if you're only user, operator knows all activity is yours.
- **Self-hosted**: you control logging but your IP visible to platforms. Blend in: allow others to use your instance (distributes suspicion).

**Video proxying** (YouTube frontends): default behavior fetches HTML from YouTube; video stream still comes from googlevideo.com (Google sees your IP). Enable proxy mode to route video through frontend server (slower, more bandwidth for operator).

**Tor .onion instances**: entire connection over Tor. Platform sees only frontend's IP (Tor exit); you connect to frontend's .onion (no exit node, hidden service). Strongest anonymity.

**Limitations**: platforms actively break frontends (API changes, rate limiting, bot detection). Frontends play cat-and-mouse. May lag behind official features. Some frontends require JavaScript (Piped) for video playback; others don't (Invidious).

**Example: YouTube via Invidious**:
1. Browse to Invidious instance (e.g., yewtu.be).
2. Search for video: Invidious queries YouTube's API using instance's IP.
3. Watch video: HTML player from Invidious. By default, video stream from googlevideo.com (Google sees your IP). Enable proxy: stream routed through yewtu.be (Google sees yewtu.be IP, not yours).
4. No Google cookies, no tracking, no account required.
