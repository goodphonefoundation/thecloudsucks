---
meta_title: "Privacy Respecting Web Browsers for PC and Mac - The Cloud Sucks"
...
## How Desktop Browsers Work (and How You’re Tracked)

Under the hood, modern browsers are **multi‑process sandboxes**:

- A **browser process** manages the UI, extensions, and network stack.
- Multiple **renderer processes** (one per site or per group of sites) execute JavaScript and render pages.
- A **GPU process** accelerates graphics.
- On some browsers, **utility processes** handle tasks like audio, network, or crash reporting.

This architecture is designed to:

- Contain compromised pages within a renderer process (sandboxing).
- Improve stability (one tab crashing doesn’t kill the entire browser).

### Same‑Origin Policy and Site Isolation

The **same‑origin policy (SOP)** is the core web security primitive:

- An *origin* is `scheme://host:port` (e.g., `https://example.com:443`).
- JavaScript from one origin cannot directly read data (DOM, cookies, localStorage) from another origin.

However, real‑world browsers allow many **cross‑origin communication channels** (postMessage, redirects, cookies, link decoration, etc.), which enable tracking even with SOP.

**Site isolation** (implemented in Chromium and partially in Firefox) strengthens this by:

- Putting different *site* instances (eTLD+1, like `example.com`) into separate processes.
- Reducing the impact of Spectre‑style side‑channel attacks between origins.

### Storage and Tracking Surfaces

A typical site can store data in many places:

- **HTTP cookies** – small key/value pairs attached to requests.
- **localStorage / sessionStorage** – origin‑scoped key/value storage in the browser.
- **IndexedDB** – large structured storage (databases).
- **Cache Storage / Service Workers** – offline content, background sync.
- **ETags / cache headers** – can be abused as tracking identifiers.

Trackers combine these with **network‑level data**:

- Request headers (User‑Agent, Accept‑Language, Referer, etc.).
- IP address and rough geolocation.
- TLS fingerprinting (cipher suite choices, ALPN, etc.) in some cases.

### Fingerprinting Basics

Fingerprinting scripts don’t rely on a single ID; they build a **high‑dimensional profile** from many attributes:

- Screen resolution, color depth, timezone, language.
- Installed fonts and emoji rendering.
- Canvas/WebGL rendering; audio context behavior.
- Platform/system details (OS, CPU architecture, touch support).
- List of enabled features and APIs (WebRTC, WebGL2, WebGPU, etc.).

The combination can often be unique even when cookies are blocked. Mitigations used by privacy‑respecting browsers include:

- **State partitioning** – separate storage per top‑level site (so trackers embedded on multiple sites can’t share the same cookie or cache entry).
- **Fingerprinting resistance** – rounding or standardizing values, blocking certain APIs, or returning generic results.
- **Blocking third‑party storage** – preventing third‑party iframes/scripts from writing cookies or localStorage.

### Practical Example: Third‑Party Tracker Across Sites

Say both `news-site.example` and `shop.example` embed `https://tracker.example/script.js`:

```text path=null start=null
You visit news-site.example:
  -> Browser loads tracker.example/script.js as a third party
  -> tracker.example sets a third-party cookie: id=abc123

Later, you visit shop.example:
  -> Browser loads tracker.example/script.js again
  -> tracker.example reads id=abc123 and knows the same browser visited both sites
```

Mitigations:

- Blocking third‑party cookies breaks this simple pattern.
- State partitioning ensures `tracker.example` sees different storage per top‑level site.
- Anti‑fingerprinting makes it harder to fall back to unique device characteristics.

### Why Different Browsers in This Guide Behave Differently

- **Mullvad Browser** – heavily locks down APIs and fingerprinting surfaces and is designed so many users look identical. Best when used with a VPN and minimal customization.
- **Firefox (+ Arkenfox)** – strong tracking protection and flexible configuration, but more tailored to *hardening a daily driver* than to blending in with a crowd.
- **Brave** – Chromium‑based with strong defaults (Shields, HTTPS upgrades, partitioning), good site compatibility, and an architecture similar to Chrome.

The recommendations and settings below are chosen to **reduce tracking surfaces** while keeping the browser usable for day‑to‑day browsing.

**Selected technical references:**

- Same‑origin policy – MDN Web Docs
- Chromium Site Isolation – https://www.chromium.org/home/chromium-security/site-isolation/
- State Partitioning – https://developer.mozilla.org/docs/Web/Privacy/State_Partitioning
...
Therefore, it is imperative that you do not modify the browser at all outside adjusting the default [security levels](https://tb-manual.torproject.org/security-settings). When adjusting the security level, you **must** always restart the browser before continuing to use it. Otherwise, [the security settings may not be fully applied](https://www.thecloudsucks.guide/articles/2025/05/02/tor-security-slider-flaw), putting you at a higher risk of fingerprinting and exploits than you may expect based on the setting chosen.
...
With the release of Firefox 128, a new setting for [privacy-preserving attribution](https://support.mozilla.org/kb/privacy-preserving-attribution) (PPA) has been added and [enabled by default](https://blog.thecloudsucks.guide/2024/07/14/mozilla-disappoints-us-yet-again-2). PPA allows advertisers to use your web browser to measure the effectiveness of web campaigns, instead of using traditional JavaScript-based tracking. We consider this behavior to be outside the scope of a user agent's responsibilities, and the fact that it is disabled by default in Arkenfox is an additional indicator for disabling this feature.
