...
## Email Clients and OpenPGP Integration

**What email clients handle**: IMAP/POP3 (fetch mail), SMTP (send mail), local storage, threading, search, attachments. OpenPGP support can be native (Thunderbird with built-in OpenPGP) or via extensions (Apple Mail + GPG Suite, webmail + Mailvelope).

**OpenPGP encryption flow**:
1. Compose message plaintext.
2. Client encrypts body with recipient's public key (asymmetric: RSA or Curve25519).
3. Optionally signs with your private key (authentication + integrity).
4. Encrypted message (`-----BEGIN PGP MESSAGE-----` block) sent via SMTP.
5. Recipient's client decrypts with their private key.

**What OpenPGP does NOT protect**: headers (To, From, Subject, Date). Metadata visible to email servers, ISP, network observers. Use Autocrypt or subject line like "..." to minimize leakage.

**Key management challenges**: discovering public keys (key servers are unreliable, spammy). Verifying keys out-of-band (fingerprint verification). Key revocation if compromised. Forward secrecy: compromise of private key exposes all past messages.

**OAuth for email**: modern auth flow where password isn't given to email client. User logs in via browser, grants token to client. Supports MFA (TOTP, hardware keys). Tokens can be revoked without changing password. Reduces phishing risk (client never sees password).

**IMAP/SMTP/POP3 security**: always use TLS (ports 993 IMAP, 465 SMTP, 995 POP3). Avoid STARTTLS on plaintext ports (110, 143, 587) due to downgrade attacks unless enforced. Email servers see plaintext unless using OpenPGP or S/MIME.

**Example: Thunderbird + OpenPGP**:
1. Generate keypair in Thunderbird (Tools → OpenPGP Key Manager).
2. Import recipient's public key (from key server or file).
3. Compose message, check "Encrypt" before sending.
4. Thunderbird encrypts body with recipient's key, signs with yours.
5. Sends encrypted message; recipient decrypts with their private key.
