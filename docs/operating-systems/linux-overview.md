...
## Linux Security Model (Sandboxing, Verified Boot, MAC)

**Monolithic kernel**: Linux kernel runs in single address space. Device drivers, filesystem, networking all in kernel mode. Vulnerability in any subsystem = full kernel compromise. Contrast: microkernels (Qubes uses Xen hypervisor, not just Linux).

**Traditional app sandboxing limitations**: desktop Linux apps run as your user. Can read your files (~/Documents, ~/.ssh, ~/.gnupg), access X11 server (keylog other apps, screenshot), use network without restrictions. No iOS/Android-style per-app permissions.

**Flatpak sandboxing**: containerized apps with restricted filesystem/device access. Portals for controlled access (file picker, camera). Issues: many apps request broad permissions (defeat sandbox), no default deny (app can request filesystem=host). Better than nothing; weaker than mobile OSes.

**SELinux vs AppArmor** (mandatory access control):
- **SELinux**: labels on all processes/files. Policy defines allowed operations. Default deny. Complex; Fedora/RHEL ship with working policies. Confines system services; user apps less restricted.
- **AppArmor**: path-based policies. Easier to write but less granular. Ubuntu/SUSE. Snap confinement uses AppArmor.

**Verified boot weaknesses**: UEFI Secure Boot verifies bootloader + kernel. But: root user can modify /boot (persistent malware). No dm-verity equivalent for rootfs by default (Android has this). Immutable distros (Silverblue, NixOS) sign system images (read-only, harder to tamper).

**X11 security problems**: any app can: monitor keystrokes in other apps, take screenshots, inject input events. Xorg runs as root (legacy). No app isolation. Wayland fixes this: compositor mediates access, apps can't spy on each other. Wayland adoption still incomplete (many DEs, some apps).

**Memory safety**: kernel + most userland written in C. Memory corruption = CVEs. Rust adoption slow (some kernel subsystems experimenting). macOS/iOS moving to Swift; Windows pushing Rust. Linux lags.

**Rolling vs fixed release security**: 
- **Rolling** (Arch, Fedora): latest software = latest security fixes + latest bugs/breakage.
- **Fixed** (Debian Stable, Ubuntu LTS): old software + backported security patches. May miss CVEs without IDs; diverges from upstream testing.

**Atomic updates** (rpm-ostree, Nix): system state transactional. Update failure = rollback, no broken system. Traditional updates: mid-upgrade power loss = potentially unbootable.

**Microcode updates**: CPU firmware patches for Spectre, Meltdown, etc. Linux-libre excludes these (ideological purity over security). Mainstream distros include (Fedora, Ubuntu auto-apply).

**Example: X11 keylogger**:
1. Malicious app requests X11 connection.
2. Xorg grants full access (no app isolation).
3. App calls XRecordExtension, logs all keystrokes system-wide.
4. Steals passwords, private messages, etc.

Wayland prevents this: compositor denies cross-app input monitoring.
...
- [Secure Data Erasure :material-arrow-right-drop-circle:](https://blog.thecloudsucks.guide/2022/05/25/secure-data-erasure)
