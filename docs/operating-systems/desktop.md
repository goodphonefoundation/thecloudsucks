...
## Linux Distribution Types (Traditional, Atomic, Anonymity-Focused)

**Traditional distributions**: package manager (apt, dnf, pacman) installs/updates software directly on system. Updates modify in-place. Failure mid-update = potentially broken system. Examples: Fedora Workstation, Arch, Debian.

**Atomic/immutable distributions**: base system read-only. Updates replace entire system image (transactional). Rollback on failure. User packages via Flatpak/containers. Examples: Fedora Silverblue (rpm-ostree), NixOS (Nix store). Benefits: reliability (atomic updates), reproducibility, easier rollback. Drawback: steeper learning curve.

**Release models**:
- **Rolling** (Arch, Tumbleweed): continuous updates. Latest software + latest bugs. Requires active maintenance (read changelogs, handle breaking changes).
- **Semi-rolling** (Fedora): freeze desktop environment per release; update kernel/apps continuously. 6-month major releases.
- **Fixed** (Ubuntu LTS): major version frozen for years. Security backports only. Stable but old software (may lack CVE fixes without IDs).

**Anonymity-focused** (Whonix, Tails): route all traffic through Tor. Whonix = two VMs (gateway + workstation); workstation can't bypass Tor even if compromised. Tails = amnesic live OS; nothing persists after reboot (unless encrypted persistent storage configured). Trade-off: Tor = slow, breaks sites, strong anonymity.

**Security-focused** (Qubes OS, Secureblue): isolation-heavy. Qubes uses Xen hypervisor; every app/task in separate VM (qube). Compromise in one qube doesn't affect others. Resource-intensive (needs 16+ GB RAM). Secureblue = hardened Fedora Atomic (GrapheneOS's hardened_malloc, security policies, Trivalent browser).

**rpm-ostree** (Fedora Silverblue): git-like system images. Base image + layered packages. Update downloads new image, applies layers, reboots into new deployment. Two deployments kept (current + previous); rollback via bootloader. Flatpak for user apps. Toolbx for traditional package manager in container.

**NixOS functional package management**: Nix language declaratively defines system. `configuration.nix` specifies packages, services, settings. `nixos-rebuild switch` atomically builds new generation. All package versions in /nix/store (immutable). Rollback = boot previous generation. Reproducible builds (same config → same system).

**Qubes security by isolation**: AppVMs (disposable VMs for tasks), TemplateVMs (shared base; AppVMs inherit). NetVMs (networking isolated; Whonix gateway = Tor). File/clipboard transfer between qubes controlled (copy/paste prompts). Color-coded window borders show qube security domain.

**Example: atomic update flow (Silverblue)**:
1. Current deployment: base image v40 + layered packages (vim, htop).
2. Run `rpm-ostree upgrade`: fetches base image v41.
3. Applies layers (vim, htop) to v41.
4. Staging complete. Reboot.
5. Bootloader offers: v41 (new, default), v40 (rollback option).
6. Boot v41. If broken, reboot and select v40.
