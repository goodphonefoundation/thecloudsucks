---
meta_title: "Remove PII with Metadata Scrubbers and Data Redaction Tools - The Cloud Sucks"
...
## Metadata in Files (Exif, GPS, Hidden Data)

**What metadata is**: information about the data, not the data itself. In images: camera model, lens, timestamp, GPS coordinates, editing software. In documents: author name, edit history, hidden comments, printer serial (yellow dots). In audio: artist, album, encoding software.

**Exif (Exchangeable Image File Format)**: standard for storing metadata in JPEG, TIFF, HEIF images. Tags include: DateTime, Make, Model, GPS coordinates, exposure settings. Modern phones embed GPS location by default (disable in camera settings).

**Privacy risks**: shared photo reveals home address (GPS), device you own (serial/model), when you were at location (timestamp). Document metadata reveals authorship (byline stripping not enough if metadata intact), organization (internal templates).

**Metadata removal**: strip before sharing. Tools parse file structure, remove metadata sections, rewrite file. Not always lossless (image quality can be preserved but metadata space reclaimed). Some formats (PNG) store metadata in ancillary chunks (easy to strip without re-encoding).

**Why blur doesn't work for text redaction**: blurred pixels still contain original data (partially). Deconvolution attacks can recover text (Unredacter tool). Always use opaque boxes over sensitive text.

**Hidden data in documents**: track changes (Word/LibreOffice), comments, embedded objects (images with own metadata), document properties (author, company). Exporting to PDF flattens some but not all (PDF metadata still exists). Use metadata cleaners or print-to-PDF (loses interactivity but safer).

**Example: GPS in photo Exif**:
```
JPEG contains Exif tag 0x8825 (GPSInfo):
- GPSLatitude: 37° 46' 29.52" N
- GPSLongitude: 122° 25' 6.84" W
- GPSAltitude: 15 meters
- GPSTimeStamp: 2025-01-15 14:32:10 UTC
```
Anyone viewing file sees exact location + time. MAT2/ExifTool removes GPSInfo tag.
