---
meta_title: "Recommended Encryption Software: VeraCrypt, Cryptomator, and OpenPGP - The Cloud Sucks"
...
## Encryption Fundamentals (Symmetric, Asymmetric, and Use Cases)

**Symmetric encryption** (AES, ChaCha20): one key encrypts and decrypts. Fast, used for bulk data (disk, file vaults). Key exchange is the hard problem.

**Asymmetric encryption** (RSA, Curve25519): public key encrypts, private key decrypts (or vice versa for signing). Slower, used for key exchange, digital signatures, and scenarios where parties don't share a secret in advance (e.g., OpenPGP email).

**At-rest encryption** (VeraCrypt, LUKS, BitLocker): protects data on disk. Adversary needs your passphrase/key or the decryption key in memory. Does not protect data once the volume is mounted.

**End-to-end encryption (E2EE)**: sender encrypts, only recipient's key can decrypt. Intermediate servers see ciphertext. Examples: OpenPGP (email), Signal Protocol (messaging).

**Transport encryption** (TLS/HTTPS): protects data in transit between you and a server, but the server sees plaintext. Not E2EE unless combined with application-level encryption.

**Authenticated encryption**: combines confidentiality (encryption) with integrity/authentication (MAC). Prevents tampering. Modern schemes: AES-GCM, ChaCha20-Poly1305.

**Key derivation functions (KDFs)**: turn a passphrase into a high-entropy key (see [passwords](passwords.md) page for PBKDF2/Argon2 details). Critical for at-rest encryption.
...
To enable BitLocker on "Home" editions of Windows, you must have partitions formatted with a [GUID Partition Table](https://en.wikipedia.org/wiki/GUID_Partition_Table) and have a dedicated TPM (v1.2, 2.0+) module. You may need to [disable the non-Bitlocker "Device encryption" functionality](https://discuss.thecloudsucks.guide/t/enabling-bitlocker-on-the-windows-11-home-edition/13303/5) (which is inferior because it sends your recovery key to Microsoft's servers) if it is enabled on your device already before following this guide.
